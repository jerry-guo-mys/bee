...
use crate::server::{saas_db_path, AppState};
...
pub(crate) fn create_router(workspace: &std::path::Path) -> Router<Arc<AppState>> {
    let db_path = saas_db_path(workspace);
    let saas_store = Arc::new(Mutex::new(SaasSqliteStore::new(&db_path).unwrap_or_else(
        |err| {
            panic!(
                "Failed to initialize SaaS SQLite store at {}: {:#}",
                db_path.display(),
                err
            );
        },
...
async fn list_tenants(
    ...
    let saas_store = saas_store
        .lock()
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let tenants = saas_store
        .list_tenants()
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    drop(saas_store);
    ...
    let summaries: Vec<TenantSummary> = tenants
        ...
        .filter(|t| {
            params
                .status
                .as_ref()
                .map_or(true, |status| &t.status.to_string() == status)
        })
        .map(|t| TenantSummary {
            id: t.id,
            name: t.name,
...
async fn create_tenant(
    ...
    Ok(Json(TenantSummary {
        ...
        organization_count: 0,
        created_at: tenant.created_at.clone(),
    }))
}
...
async fn get_tenant(
    Path(id): Path<String>,
    Extension(saas_store): Extension<SaasStore>,
) -> Result<Json<TenantDetailResponse>, (StatusCode, String)> {
    let saas_store = saas_store
        .lock()
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let tenant = saas_store
        .get_tenant(&id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or_else(|| (StatusCode::NOT_FOUND, "Tenant not found".to_string()))?;
    let organizations = saas_store
        .list_organizations(Some(&id))
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .into_iter()
...
async fn suspend_tenant(
    ...
    let saas_store = saas_store
        ...
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    saas_store
        .update_tenant_status(&id, &TenantStatus::Suspended)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
async fn restore_tenant(
    ...
    let saas_store = saas_store
        ...
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    saas_store
        .update_tenant_status(&id, &TenantStatus::Active)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(StatusCode::NO_CONTENT)
}
...
async fn archive_tenant(
    ...
    let saas_store = saas_store
        ...
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    saas_store
        .update_tenant_status(&id, &TenantStatus::Archived)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(StatusCode::NO_CONTENT)
}
...
async fn list_organizations(
    ...
    Extension(saas_store): Extension<SaasStore>,
) -> Result<Json<OrganizationListResponse>, (StatusCode, String)> {
    let saas_store = saas_store
        .lock()
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let organizations = saas_store
        .list_organizations(params.tenant_id.as_deref())
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
) -> Result<Json<OrganizationSummary>, (StatusCode, String)> {
    let now = Utc::now().to_rfc3339();
    let org = bee::saas::Organization {
        id: Uuid::new_v4().to_string(),
...
async fn list_teams(
    ...
    Extension(saas_store): Extension<SaasStore>,
) -> Result<Json<TeamListResponse>, (StatusCode, String)> {
    let saas_store = saas_store
        .lock()
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let teams = saas_store
        .list_teams(params.organization_id.as_deref())
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    let summaries: Vec<MemberSummary> = memberships
        ...
        .filter(|m| {
            params
                .role
                .as_ref()
                .map_or(true, |role| &m.role.to_string() == role)
        })
        .map(|m| MemberSummary {
            id: m.id,
...
async fn list_audit_logs_enhanced(
