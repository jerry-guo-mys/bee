...
mod tests {
    ...
    async fn test_full_workflow_execution() {
        ...
        let engine = WorkflowEngine::new(Arc::new(queue), executor.clone());

        let workflow = WorkflowBuilder::new("Integration Test")
            .user_id("user1".to_string())
            .task(
                "a",
                BackgroundTask::new("user1".to_string(), "Task A".to_string()),
            )
            .task(
                "b",
                BackgroundTask::new("user1".to_string(), "Task B".to_string()),
            )
            .task(
                "c",
                BackgroundTask::new("user1".to_string(), "Task C".to_string()),
            )
            .sequential("a", "b")
            .sequential("b", "c")
            .build()
    ...
    async fn test_fallback_on_failure() {
        ...
        let engine = WorkflowEngine::new(Arc::new(queue), executor);

        let workflow = WorkflowBuilder::new("Fallback Test")
            .user_id("user1".to_string())
            .task(
                "main",
                BackgroundTask::new("user1".to_string(), "Main task".to_string()),
            )
            .task(
                "fallback",
                BackgroundTask::new("user1".to_string(), "Fallback task".to_string()),
            )
        ...
        assert!(matches!(
            status,
            Some(WorkflowStatus::Running | WorkflowStatus::Failed)
        ));
    }
}
